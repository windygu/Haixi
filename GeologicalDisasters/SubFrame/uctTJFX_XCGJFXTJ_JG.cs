...
{
    public partial class uctTJFX_XCGJFXTJ_JG : UserControl
    {
        public delegate void uctTJFX_XCGJFXTJ_JGEventHandler(string  p_strGJID,string p_strGJZB);
        public event uctTJFX_XCGJFXTJ_JGEventHandler uctTJFX_XCGJFXTJ_JGEvent;
        ...
        private void dataGridViewX1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (m_MouseEventArgs.Button == MouseButtons.Left)
                {
                    ...
                    if (uctTJFX_XCGJFXTJ_JGEvent != null && m_DataGridView.SelectedRows.Count > 0)
                    {

                        uctTJFX_XCGJFXTJ_JGEvent(m_DataGridView.SelectedRows[0].Cells["轨迹标识"].Value.ToString(), m_DataGridView.SelectedRows[0].Cells["轨迹坐标"].Value.ToString());//将轨迹id传出去
                    }
                }
                    ...
                    else
                    {
                        ...
                        if (m_DataGridView.SelectedCells.Count > 0)
                        {
                            if (m_DataGridView.Columns[m_DataGridView.SelectedCells[0].ColumnIndex].Name == "手机")
                            {
                                ...
                                txtName.Text = (string)m_DataGridView.Rows[m_DataGridView.SelectedCells[0].RowIndex].Cells["执法人员姓名"].Value;
        ...
        private void uctTJFX_XCGJTJ_JG_Load(object sender, EventArgs e)
        {
        ...
        private void btnExportData_Click(object sender, EventArgs e)
        {
            ...
            m_DataAccess_.OutputExcel((DataGridView)m_DataGridView, "执法网格", m_strFileName);
        }
